    ...
    if items then
        for name, data in pairs(items) do
            for _, v in pairs(data) do
                if v.slot then
                    options[#options+1] = {id = name .. v.slot, title = v.label, description = string.format('Durability: %s', v.metadata.durability .. '%'), serverEvent = 'raze_weaponrepair:startweaponrepair', args = {slot = v.slot, name = name}}
                end
            end
        end
...
RegisterNetEvent('raze_weaponrepair:repairitem', function(name)
    if lib.progressBar({
        ...
        TriggerServerEvent('raze_weaponrepair:fixweapon')
    end
end)
